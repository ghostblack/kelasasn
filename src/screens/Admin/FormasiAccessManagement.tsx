...
import { FormasiAccessCode } from '@/types';
import {
  generateFormasiCode,
  getAllFormasiCodes,
  updateFormasiCodeStatus,
  deleteFormasiCode,
} from '@/services/formasiAccessCodeService';
...
export const FormasiAccessManagement: React.FC = () => {
  const { toast } = useToast();
  const { user } = useAuth();
  const [codes, setCodes] = useState<FormasiAccessCode[]>([]);
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const codesData = await getAllFormasiCodes();
      setCodes(codesData);
    } catch (error: any) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const code = await generateFormasiCode(
        formData.maxUses,
        user.uid,
        expiryDate
      );
      ...
      toast({
        title: 'Berhasil',
        description: `Kode akses berhasil dibuat: ${code}`,
      });

      setShowDialog(false);
      setFormData({ maxUses: 1, expiryDate: '' });
      loadData();
    } catch (error) {
      console.error('Error generating code:', error);
      toast({
        title: 'Error',
        description: 'Gagal membuat kode akses',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleToggleStatus = async (codeId: string, currentStatus: boolean) => {
    try {
      await updateFormasiCodeStatus(codeId, !currentStatus);
      toast({
        title: 'Berhasil',
  ...
  const handleDelete = async (codeId: string) => {
    if (!confirm('Yakin ingin menghapus kode akses ini?')) return;

    try {
      await deleteFormasiCode(codeId);
      toast({
        title: 'Berhasil',
        description: 'Kode akses berhasil dihapus',
      });
      loadData();
    } catch (error) {
      ...
      toast({
        title: 'Error',
        description: 'Gagal menghapus kode akses',
        variant: 'destructive',
      });
    }
  };
    ...
    <div className="space-y-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-gray-100 pb-10">
        <div>
           <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] font-bold text-gray-400 capitalize tracking-wider">Premium Features</span>
           </div>
           <h1 className="text-4xl font-extrabold text-gray-900 tracking-tight leading-none">
             Akses <span className="text-gray-400 font-medium ml-2">Formasi CPNS</span>
           </h1>
           <p className="text-sm text-gray-500 mt-4 leading-relaxed max-w-xl">
             Generate dan kelola kode akses eksklusif untuk fitur Pencarian Formasi CPNS. User yang menggunakan kode ini akan mendapat akses selama 1 minggu.
           </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {codes.map((item) => (
            <Card key={item.id} className="bg-white border border-gray-100 rounded-none shadow-sm overflow-hidden hover:border-gray-200 transition-all group">
              ...
              <div className="p-6 space-y-6">
                <div className="relative group/code">
                  <div className="absolute -inset-2 bg-blue-50/0 group-hover/code:bg-blue-50/50 transition-all duration-300 -z-10" />
                ...
                <div className="grid grid-cols-2 gap-px bg-gray-100 border border-gray-100">
                  <div className="bg-white p-3">
                    ...
                    <p className="text-xs font-black text-gray-900">
                      {item.currentUses} <span className="text-gray-300 font-medium">/</span> {item.maxUses}
                    </p>
                  </div>
                  <div className="bg-white p-3">
                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1">Status</p>
                    <p className="text-xs font-black text-gray-900">
                      {item.currentUses >= item.maxUses ? 'EXHAUSTED' : 'AVAILABLE'}
                    </p>
                  </div>
                  <div className="bg-white p-3 col-span-2 border-t border-gray-100">
                     ...
                     <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-gray-900">{item.createdAt.toLocaleDateString('id-ID')}</span>
                        <span className="text-xs font-medium text-gray-400">
                          {item.expiryDate ? item.expiryDate.toLocaleDateString('id-ID') : 'No Expiry'}
                        </span>
                     </div>
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    className={`flex-1 rounded-none text-[10px] font-bold uppercase tracking-widest h-10 border border-gray-100 transition-all ${
                      item.isActive ? 'text-gray-400 hover:text-orange-600 hover:bg-orange-50/50' : 'text-blue-600 hover:bg-blue-50/50 bg-blue-50/30'
                    }`}
                    onClick={() => handleToggleStatus(item.id, item.isActive)}
                  >
                    <Power className="w-3.5 h-3.5 mr-2" />
                    {item.isActive ? 'Deactivate' : 'Activate Area'}
                  </Button>
                  <Button
                    variant="ghost"
                    size="sm"
          ...
          {codes.length === 0 && (
            <div className="col-span-full text-center py-24 bg-white border border-dashed border-gray-200">
               ...
               <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">No access codes found</p>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Generate Kode Akses Formasi</DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Jumlah user yang dapat menggunakan kode ini"
              />
              <p className="text-[10px] text-gray-400">Setiap user yang menggunakan kode ini secara otomatis mendapat akses pencarian selama 1 minggu (7 hari).</p>
            </div>

            <div className="space-y-2">
              <Label>Batas Waktu Tukar Kode Akses (Opsional)</Label>
              <Input
                type="date"
              ...
              />
              <p className="text-[10px] text-gray-400">Tanggal terakhir kode ini bisa dipakai (berbeda dengan masa aktif akses 7 hari).</p>
            </div>
          </div>
